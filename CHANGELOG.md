...
1. 补齐游戏真实登录验收的 Windows 服务运行配置：服务启动器显式加载发布用户或机器级 `RHOSPITAL_RELEASE_AUTH_TOKEN_FILE`，token 前置检查增加游戏签发格式和至少两小时剩余有效期校验，并移动到代码切换与完整 Maven 测试之前。缺少、无效或临近到期的受控 token 会在改变工作区版本、耗时测试、镜像构建和全部生产动作前立即停止，日志继续隐藏 token 内容。

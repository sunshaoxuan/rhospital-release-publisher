...
发布器现已按目标选择生产 Docker Server。游戏继续使用 `SSH178` 和 `178.239.117.99`，论坛使用 `FORUM_PRD2` 和 `92.113.124.185`。目标专用配置优先于 Windows 服务遗留的全局 `SSH178` 环境变量。页面配置请求使用序号门禁，较早响应无法覆盖当前目标。

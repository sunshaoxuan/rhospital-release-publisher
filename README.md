...
发布结果包含首次失败、回滚依据、警告和下一步排查方向。辅助分类通过官方 TypeSafe SDK 调用 Jev，需配置已获访问权限的 TypeSafe 凭据；未配置时显示待配置状态。构造历史可点击“复核发布结果”。配置、数据边界与验证方法见 [发布诊断说明](docs/RELEASE_DIAGNOSTICS.md)。
...
首次启动或依赖锁文件更新后，在发布器目录执行 `npm ci` 安装固定版本依赖，再启动服务。本版本新增官方 TypeSafe SDK；当前运行目录已安装。

...
发布结果现在包含首次失败、回滚依据、警告和下一步排查方向，并可通过现存 Qwen 服务执行 OpenJev 式辅助分类。构造历史可点击“复核发布结果”。配置、数据边界与验证方法见 [发布诊断说明](docs/RELEASE_DIAGNOSTICS.md)。
...
- 发布器全局串行执行任务。已有任务处于 `RUNNING`、`CANCELLING`、`RECOVERING` 或尚在清理时，新执行请求返回 HTTP 409，错误码为 `PUBLISHER_JOB_ACTIVE`。游戏、论坛、dry run、多标签页和 API 调用共用这一门禁。
- 页面提交请求期间以及任务运行期间禁用“执行流程”，避免重复点击创建任务。任务结束后恢复按钮；提交失败时释放提交锁。
- 轮询在请求前和响应后确认当前任务 ID，失效任务的响应不能更新进度或建立下一次轮询。状态读取暂时失败时保留任务并自动重试。
- 页面刷新后从服务端恢复当前唯一的活动任务，继续显示真实步骤、日志、计时和取消入口。刷新浏览器不改变后台任务状态。
- 更新发布器时等待所有现有任务结束，再由已有的空闲版本检查机制重启。禁止为更新控制台中断正在执行的发布。
- 浏览器回归使用 `node test/fixtures/execution-ui-server.cjs`，地址为 `http://127.0.0.1:18787/`。该服务仅提供模拟 API 和真实页面资源，不加载发布引擎或生产配置。
...
- `master` 的 pre-push 门禁使用 `origin/master` 作为基线，只在 dry run 中接受基于该基线的拟议快进提交。分叉提交、回退提交和正式执行仍要求目标提交已经属于发布分支。
...
8. Chrome 软件 WebGL 使用 Chromium 当前支持的 SwANGLE 启动组合 `--use-gl=angle --use-angle=swiftshader-webgl --enable-unsafe-swiftshader`。每个 CDP 请求使用独立硬超时。`Framebuffer Unsupported`、CDP 超时、目标关闭和浏览器进程异常会使用全新 Chrome profile 隔离复测一次。复测会记录失败证据和尝试次数。重复基础设施异常、应用运行错误、资源错误和网络错误继续失败关闭。
...
- 总进度阶段严格按计划中的实际执行顺序聚合。本次影响评估动态加入的检查归入其相邻阶段，不会被追加到固定阶段之后造成视觉跳步。
...
- 页面首次加载、刷新页面或切换游戏与论坛发布目标时，发布分支统一默认选择本地 `master`。工作区当前分支不会覆盖该默认值；只有点击 `刷新提交` 时才保留用户已经手工选择的其他分支。
